...
import { generateSessionId, formatMessage, escapeHtml } from './utils';
...
export default function App() {
  ...
  const connect = useCallback(() => {
    ...
    socket.onopen = () => {
      setIsConnected(true);
      console.log('Connected to PulseBot');
    };
  ...
  const removeToolCallIndicators = () => {
    ...
    setMessages(msgs => msgs.map(m => 
        (m.type === 'llm_thinking' && m.status === 'started') ? {
            ...m,
            status: 'completed'
        } : m
    ));
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50 text-gray-900 font-sans">
      ...
      <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center gap-3">
        <img src="/static/assets/pulsebot_logo_pink.svg" alt="PulseBot" className="h-7 w-auto block" />
        <div className="ml-auto flex items-center gap-2 text-xs text-gray-600">
          <span className={`w-2 h-2 rounded-full ${isConnected ? 'bg-green-600' : 'bg-gray-400'}`}></span>
          <span>{isConnected ? 'Connected' : 'Disconnected'}</span>
        </div>
      </header>

      {/* Chat Container */}
      <main className="flex-1 flex flex-col max-w-[900px] w-full mx-auto p-6 gap-4 overflow-hidden">
        {/* Messages Area */}
        <div className="flex-1 overflow-y-auto flex flex-col gap-4 pr-2 messages-scroll pb-4">
          
          {messages.length === 0 && Array.from(activeToolCalls.values()).length === 0 && Array.from(activeLlmThinking.values()).length === 0 && !isWaitingForResponse ? (
                ...
                if (item.type === 'message') {
                  ...
                  return (
                    <div key={msg.id} className={`flex gap-3 animate-fade-in ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                      <div className={`w-8 h-8 rounded shrink-0 flex items-center justify-center text-xs font-semibold text-white ${msg.role === 'assistant' ? 'bg-pink-500' : 'bg-gray-700'}`}>
                        {msg.role === 'assistant' ? 'P' : 'U'}
                      </div>
                      <div className="w-fit max-w-[70%]">
                        <div className={`border rounded px-4 py-3 break-words markdown-body ${msg.role === 'user' ? 'bg-gray-800 text-white border-gray-800 markdown-user' : 'bg-white border-gray-200'} text-[14px]`}
                             dangerouslySetInnerHTML={{ __html: formatMessage(msg.text) }}>
                        </div>
                        <div className={`text-[11px] text-gray-500 mt-1 ${msg.role === 'user' ? 'text-right' : ''}`}>
                          {msg.time}
                        </div>
                      </div>
                    </div>
                  ...
                  const hasDetails = tool.resultPreview && (tool.resultPreview.length > 50 || tool.resultPreview.includes('\n') || !isSuccess);
                  ...
                  return (
                    <div key={tool.id} className={wrapperClass}>
                      <div className="flex items-start gap-2.5 w-full">
                        ...
                        <div className="flex flex-col flex-1">
                          <div>
                            ...
                            {tool.argsSummary && <span className={`font-mono text-xs ml-1 whitespace-pre-wrap ${isSuccess ? 'text-[#276749]' : 'text-gray-600'}`}>{tool.argsSummary}</span>}
                          </div>
                        </div>
                        <span className={`text-xs whitespace-nowrap ml-auto ${isSuccess ? 'text-[#38B2AC]' : 'text-gray-500'}`}>
                          {isStarted ? 'running...' : isSuccess ? `${tool.durationMs}ms` : 'failed'}
                        </span>
                      </div>
        ...
        <div className="bg-white border border-gray-200 rounded p-[12px] flex gap-[12px] items-end focus-within:border-pink-primary focus-within:ring-[2px] focus-within:ring-pink-light transition-all">
          ...
          ></textarea>
          <button 
            className="h-8 px-4 bg-pink-primary hover:bg-pink-hover text-white border-none rounded text-[14px] font-medium cursor-pointer transition-colors flex items-center gap-[6px] disabled:bg-gray-400 disabled:!text-white disabled:cursor-not-allowed shrink-0"
      ...
      {/* Toast */}
      <div className={`fixed bottom-6 left-1/2 -translate-x-1/2 px-5 py-3 rounded text-sm text-white shadow-lg transition-all duration-300 z-50 ${toast.visible ? 'translate-y-0 opacity-100' : 'translate-y-[100px] opacity-0'} ${toast.isError ? 'bg-red-500' : 'bg-gray-900'}`}>
        {toast.message}
      </div>
    </div>
  );
